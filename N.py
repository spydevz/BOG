...
for method in all_methods:
    ...
    async def cmd(ctx, ip: str = None, port: int = None, duration: int = None, method_name=method):
...
async def dhelp(ctx):
    ...
    for m in all_methods:
        embed.add_field(name=f"!{m} <ip> <port> <time>", value="Lanza ataque con método", inline=False)
    await ctx.send(embed=embed)
